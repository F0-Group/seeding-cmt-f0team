...
function generateFeedback() {
    ...
    if (category in categories) {
        const feedback = categories[category].slice(0, count).map(sentence => sentence.replace("[A]", productName));
...
function showSuggestions() {
    ...
    if (input) {
        ...
        if (filteredSuggestions.length > 0) {
            ...
            // Hiển thị thông báo khi có gợi ý
            showNotification("Suggestions updated", "green-100", "green-500", "green");
        } else {
            // Hiển thị thông báo khi không tìm thấy gợi ý
            showNotification("No suggestions found", "red-100", "red-500", "red");
        }
    } else {
        // Hiển thị thông báo khi không có đầu vào
        showNotification("Please enter a product name", "yellow-100", "yellow-500", "yellow");
    }
}
...
function refreshPage() {
    location.reload();
    displayComments();
    showNotificatiofb("Refresh", "blue-100", "blue-500", "blue");
}
function refreshPage() {
